...
function markMainAreaPromise() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    console.log(
      "Body { width:" + _body.width + " height: " + _body.height + " }"
    );
    ...
    $("div.leaf").each(function() {
      ...
      if (_parent.children(".leaf").length >= 2) {
        ...
        if (_parent.find(".collect-btn").length > 0) {
          ...
          _parent.append('<div class="collect-btn">采集</div>');
        }
      } else {
      }
    });
  ...
  function saveDom(ele, cate) {
    ...
    let item = {
      ...
      relativeTextPercentage:
        _innerTextLength /
        _parent.prop("innerText").replace(/(\n)?Spider(\n)?/gi, "").length,
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  switch (message.type ? message.type : message.message) {
    ...
    case "markMainArea":
      regionFocus().then(() => {
        console.log("Mark main area done!");
      });
      break;
    case "autoRefresh":
      autoRefresh();
...
function init() {
  ...
  // contentInit();
  // markMainArea();
  let pageClassifyOutput = pageClassify(true);
  addControlPanel(pageClassifyOutput);
  // markMainAreaPromise();
}
